...
import { Slope, KlineState } from "../src/indicator/slope";
...
describe('Slope', () => {
  it('匀速上涨', () => {
    ...
    const slope = new Slope({ epsilon: 0.003 });
    let slopeValue = KlineState.Mixed;
    klines.forEach(kline => {
      slopeValue = slope.add(kline);
    })
    expect(slopeValue).toEqual(KlineState.Mixed);
  });
  it('加速上涨', () => {
    ...
    expect(slopeValue).toEqual(KlineState.AcceleratingUp);
  });
  it('减速上涨', () => {
  ...
  it('加速下跌', () => {
    ...
    expect(slopeValue).toEqual(KlineState.AcceleratingDown);
  });
  it('减速下跌', () => {
    ...
    expect(slopeValue).toEqual(KlineState.DeceleratingDown);
  });
  it('匀速下跌', () => {
    ...
    expect(slopeValue).toEqual(KlineState.SteadyDown);
  });
});
