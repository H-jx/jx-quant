...
//!
//! Build with: `cargo build --release --features ffi-python`
    ...
    fn poll_signals<'py>(&self, py: Python<'py>) -> PyResult<Vec<PyObject>> {
        ...
        for s in sigs {
            let action = match s.action {
                crate::Action::Buy => "BUY",
                crate::Action::Sell => "SELL",
                crate::Action::Hold => "HOLD",
            };
...
fn parse_action_str(s: &str) -> Option<crate::Action> {
    match s.to_ascii_uppercase().as_str() {
        "BUY" => Some(crate::Action::Buy),
        "SELL" => Some(crate::Action::Sell),
        "HOLD" => Some(crate::Action::Hold),
        _ => None,
    }
}
...
impl FuturesBacktest {
    ...
    fn apply_signal(&self, action: &str, price: f64, margin: f64) -> PyResult<()> {
        let a = parse_action_str(action)
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("invalid action"))?;
